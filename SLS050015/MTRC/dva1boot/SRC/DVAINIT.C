...
int RAMCheck( void )
{
		...
		for( cnt=0; cnt<RAM_SIZE/2; cnt++,p++ ){
			...
			*p = data1;
			if( data2!=0x55AA ){
				printf("RAM Check Error Address %lX write=55AA read=%04X\r\n", p, data2 );
				return 1;
			}
//			dvaIOOut( ((unsigned long)p & 0x1FE00)>>9, 0x00FF );
		}
		dvaIOOut( 0x02, 0x00FF );
		p = (unsigned short *)RAMTOP_ADDRESS; /* RAM Top */
		for( cnt=0; cnt<RAM_SIZE/2; cnt++,p++ ){
			data1 = *p;
			*p = 0xAA55;
			...
			*p = data1;
			if( data2!=0xAA55 ){
				printf("RAM Check Error Address %lX write=AA55 read=%04X\r\n", p, data2 );
				return 1;
			}
//			dvaIOOut( ((unsigned long)p & 0x1FE00)>>9, 0x00FF );
		}
...
//				return 1;
//			}
//			dvaIOOut( ((unsigned short)p & 0xFF00)>>8, 0x00FF );
//		}
//		p = (unsigned short *)RAMTOP_ADDRESS; /* RAM Top */
//		for( cnt=0; cnt<RAM_SIZE/2; cnt++,p++ ){
//			data1 = *p;
...
/************************************************************************
*																		*
*	Function name	<< DUALRAMCheck >>	
*					‰ŠúÝ’è
*	Syntax			DUALRAMCheck( void );
*	Argument		none												*
*	Author			TOA Corporation  K.Honda							*
*	Revision		a00 2000/06/27										*
*																		*
*************************************************************************/
int DUALRAMCheck( void )
{
		volatile unsigned char	*p;
		long	cnt;
		unsigned char	data2;

		p = (unsigned char *)&DUALBOOT; /* RAM Top */
		for( cnt=0; cnt<sizeof(DUALBOOT); cnt++,p++ ){
			*p = 0x55;
			data2 = *p;
			if( data2!=0x55 ){
				printf("DUALRAM Check Error Address %lX write=55 read=%02X\r\n", p, data2 );
				return 1;
			}
		}
		p = (unsigned char *)&DUALBOOT; /* RAM Top */
		for( cnt=0; cnt<sizeof(DUALBOOT); cnt++,p++ ){
			*p = 0xAA;
			data2 = *p;
			if( data2!=0xAA ){
				printf("DUALRAM Check Error Address %lX write=AA read=%02X\r\n", p, data2 );
				return 1;
			}
		}
		p = (unsigned char *)&DUALBOOT; /* RAM Top */
		for( cnt=0; cnt<sizeof(DUALBOOT); cnt++,p++ ){
			*p = 0xFF;
			data2 = *p;
			if( data2!=0xFF ){
				printf("DUALRAM Check Error Address %lX write=FF read=%02X\r\n", p, data2 );
				return 1;
			}
		}
		p = (unsigned char *)&DUALBOOT; /* RAM Top */
		for( cnt=0; cnt<sizeof(DUALBOOT); cnt++,p++ ){
			*p = 0x00;
			data2 = *p;
			if( data2!=0x00 ){
				printf("DUALRAM Check Error Address %lX write=00 read=%02X\r\n", p, data2 );
				return 1;
			}
		}
...
int ROMCheck( void )
{
		...
		if( data ){
			printf("ROM Check Error SUM = %02X\r\n", data );
		}
		return data;
}
