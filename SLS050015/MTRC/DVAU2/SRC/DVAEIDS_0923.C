...
void dvauRTIFCtrl( void )
{
	...
	case 4:		// Message Structure 完成まで15byte追加で読み込み
		...
		while(dequeue(&rtif_recv_q,&c))
		{
			...
			rtifarea[rtifarea_recvsize++] = c;
			if(rtifarea_recvsize == 256) rtifarea_recvsize == 255;
			if( ( rtifarea_recvsize - rtifarea_delimiter_pos ) == 16 )
			{
				rtif_ph = 5;
				break;
			}
	...
	case 5:
		...
		opener = rtifarea[0] - 0x20;
		closer = (unsigned int)rtifarea[rtifarea_delimiter_pos+1];
		closer = closer << 8;
		closer += (unsigned int)rtifarea[rtifarea_delimiter_pos+2];
		closer -= 0x2020;
		closer = closer >> 10;
		if( (unsigned int)opener == closer )
		{	// openerとcloserが一致すればmessage structureと認める。
