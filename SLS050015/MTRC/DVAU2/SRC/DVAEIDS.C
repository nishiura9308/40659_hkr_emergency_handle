...
// for RTIF
static	const unsigned char	rtifcmd_modemsense[6] = { 0x52,0x46,0x46,0x41,0x45,0x0d };	// "RFFAE"
																// 1+1+1+12+11+1+2+1=30
static	const unsigned char	rtifrcv_example[30] = {	0x2c,		// 1
													0x26,		// 1
													0x52,		// 1
...
static	const unsigned char	CRC_ERROR[11] = { 0x43,0x52,0x43,0x20,0x45,0x72,0x72,0x6f,0x72,0x0d }; // "CRC Error"
static	const unsigned char	TIMEOUT_ERROR[10] = { 0x54,0x69,0x6d,0x65,0x20,0x4F,0x75,0x74,0x0d }; // "Time Out"


int rtif_ph = 0;					/* RTIF制御フェーズ */
int rtif_status = 0;				/* RTIF 0:未接続/1:接続中 */
int rtifarea_recvsize = 0;			/* RTIF受信サイズ */
int rtifarea_delimiter_pos = 0;		/* RTIF受信デリミタ検出位置 */
unsigned char rtifarea[256];		/* RTIF受信バッファー */
int rtifmessage_flag = 0;			/* RTIFバッファ状態フラグ 0:データ無し/1:データ有り/2:編成間通信へ送出済み */
unsigned char rtifmessage[256];		/* RTIFメッセージ */
int rtifmessage_size = 0;			/* RTIFメッセージサイズ */
...
QUEUE rtif_recv_q;					/* RTIF受信キュー */

void queueinit(QUEUE*);
int enqueu(QUEUE*,unsigned char);
int dequeue(QUEUE*,unsigned char*);
void dvauRTIFCtrl( void );
...
void dvauEIDSInitialize( void )
{
		...
		queueinit(&rtif_recv_q);		// RTIF 受信キュー初期化
}
...
//  値のプッシュ
int enqueue(QUEUE* pQueue,unsigned char value)
{
//  lastの次が、firstならば、失敗
...
//  値のポップ
int dequeue(QUEUE* pQueue,unsigned char* pValue)
{
    //  キューにデータが一つもないなら、失敗
...
void dvauRTIFCtrl( void )
{
	...
	unsigned char c;
	unsigned long crc;
	unsigned char crcdiv6[6];

	switch(rtif_ph)
	{
	case 0:		// LTEmodem検出
		sendp = (unsigned char *)&rtifcmd_modemsense;	// "RFFAE"
		sendlen = sizeof(rtifcmd_modemsense);
		...
		timedata.rtiftimer = 25;		// 250mS TimeOut
		rtif_status = 0;				// 非接続中
		rtif_ph = 1;
		break;

	case 1:
		if( timedata.rtiftimer==0 ){
			rtif_ph = 0;				// 250mS TimeOut
			break;
		}
		while(dequeue(&rtif_recv_q,&c))
		{
			if( ( c == 0x30 ) || ( c == 0x38 ))
			{							// 応答有り
				timedata.rtiftimer = 25;		// 250mS TimeOut
...
#if 0
			if( rtifarea_recvsize != 0 )
			{
				sendp = (unsigned char *)&TIMEOUT_ERROR;
				sendlen = sizeof(TIMEOUT_ERROR);
	...
	//			while( sendcnt!=sendlen );
...
#endif
			rtifarea_recvsize = 0;		// キャラクタ間タイムアウトでバッファクリア
			timedata.rtiftimer = 25;	// 250mS TimeOut
			break;
		}
		while(dequeue(&rtif_recv_q,&c))
		{
			...
			if( c == 0x5f )
			{	//	デリミタ
				rtifarea_delimiter_pos = rtifarea_recvsize;
				rtifarea[rtifarea_recvsize++] = c;
				if(rtifarea_recvsize == 256) rtifarea_recvsize == 255;
				rtif_ph = 4;
				break;
			}
			else
		...
		break;
	case 4:		// Message Structure 完成まで15byte追加で読み込み
		if( timedata.rtiftimer==0 ){
#if 0
			if( rtifarea_recvsize != 0 )
			{
	...
	//			while( sendcnt!=sendlen );
		...
		while(dequeue(&rtif_recv_q,&c))
		{
			...
			rtifarea[rtifarea_recvsize++] = c;
			if(rtifarea_recvsize == 256) rtifarea_recvsize == 255;
			if( ( rtifarea_recvsize - rtifarea_delimiter_pos ) == (16+2) ) // CRC2 XX XX XX XX XX XX の代わりに00 00 0D 0A 0D 0A 0D 0Aとなり＋２とする。
			{
				rtif_ph = 5;
				break;
			}
	...
	case 5:
#if 0
		for( i = 0 ; i < sizeof(rtifrcv_example) ; i++ )
		{
			rtifarea[i] = rtifrcv_example[i];
		}
		crc = crc32(&rtifarea[1],26);
		rtifarea[sizeof(rtifrcv_example)] = (unsigned char)((crc >> 24) & 0xff);
		rtifarea[sizeof(rtifrcv_example)+1] = (unsigned char)((crc >> 16) & 0xff);
		rtifarea[sizeof(rtifrcv_example)+2] = (unsigned char)((crc >> 8) & 0xff);
		rtifarea[sizeof(rtifrcv_example)+3] = (unsigned char)((crc) & 0xff);

		rtifarea_recvsize = sizeof(rtifrcv_example)+12;	// +CRC12byte
#endif

		crc = crc32(&rtifarea[0],rtifarea_recvsize-12-2);
		crcdiv6[0] = (unsigned char)( ( crc >> 26 ) & 0x3f) + 0x30;
		crcdiv6[1] = (unsigned char)( ( crc >> 20 ) & 0x3f) + 0x30;
		crcdiv6[2] = (unsigned char)( ( crc >> 14 ) & 0x3f) + 0x30;
		crcdiv6[3] = (unsigned char)( ( crc >> 8 ) & 0x3f) + 0x30;
		crcdiv6[4] = (unsigned char)( ( crc >> 2 ) & 0x3f) + 0x30;
		crcdiv6[5] = (unsigned char)( ( crc << 4 ) & 0x3f) + 0x30;

//		if( ( rtifarea[rtifarea_recvsize-12-2] == crcdiv6[0] ) &&
//			( rtifarea[rtifarea_recvsize-11-2] == crcdiv6[1] ) &&
//			( rtifarea[rtifarea_recvsize-10-2] == crcdiv6[2] ) &&
//			( rtifarea[rtifarea_recvsize-9-2] == crcdiv6[3] ) &&
//			( rtifarea[rtifarea_recvsize-8-2] == crcdiv6[4] ) &&
//			( rtifarea[rtifarea_recvsize-7-2] == crcdiv6[5] ) )
//		{	// crc1が一致すればmessage structureと認める。
		if( (rtifmessage_flag == 0) || (rtifmessage_flag == 2) )
		{
			rtifmessage_size = rtifarea_recvsize;
			for( i = 0 ; i <  rtifmessage_size ; i++ )
			{
				rtifmessage[i] = rtifarea[i];
			}
			rtifmessage_flag = 1;		// 空いていれば編成間通信へ流す。
#if 0
			sendp = (unsigned char *)&rtifmessage;
			sendlen = rtifmessage_size;
...
//		{
#if 0
			sendp = (unsigned char *)&CRC_ERROR;
			sendlen = sizeof(CRC_ERROR);
