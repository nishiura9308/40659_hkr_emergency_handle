...
extern unsigned char rtifarea[192];		/* RTIF受信バッファー */
extern int rtifarea_recvsize;			/* RTIF受信サイズ */
...
static	unsigned char	rttfsendbuff[256];	// RTTF用送信バッファー*/
...
*************************************************************************/
void dvauRTTFSend( unsigned char *sp,  unsigned char *senddata, int length)
{
		int		i;
		...
		for( i=0; i<length; i++ ){
			rttfsendbuff[2+i] = senddata[i];
		}
		max_receive_time = (2+length)*10/45;
		timedata.CommRecvTimer = max_receive_time;
		dvauSend( (unsigned char *)rttfsendbuff, 2+length );
}
...
//				sendpolling.kind = 'N';		// 通常送信
				...
				if( adrTable[adrcnt] == 14 )	// RTTF
				{
					if( rtifarea_recvsize != 0)
					{
						dvauRTTFSend( (unsigned char *)&sendpolling,(unsigned char *)rtifarea, rtifarea_recvsize );
					}
					else
					{
						sendactive = 1;
