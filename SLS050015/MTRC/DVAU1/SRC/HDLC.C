...
*************************************************************************/
/*. --------------------------【機能詳細】----------------------------[@@@S]
　・送受信データ１バイトごとに割り込みを発生させる
　・受信データはバッファーへ格納し、終了割り込みによって受信処理へ渡す
	------------------------------------------------------------------[@@@E]*/
void    HDLC_Int(void)
{
...
/************************************************************************
	...
	Argument		char *buff		i	: 送信バッファ
					int send_length	i	: 送信データサイズ
	Author			TOA Corporation  K.Honda
...
*************************************************************************/
/*. --------------------------【機能詳細】----------------------------[@@@S]
　・ＴＭＳへ応答するデータは、一度この関数によって送信信号を
　送信できる状態にする。
　・Opening PAD を送信するため、タイマー割り込みによって時間待ちする
	------------------------------------------------------------------[@@@E]*/
...
/************************************************************************
	...
	Function name	<< HDLC_Initialize >>	
					初期設定
	Syntax			HDLC_Initialize( baud, lowadr, send_length);
	Argument		int baud		i	:	通信速度 2400/4800/9600/19200
					char lowadr		i	:	送信アドレス（下位）
					int send_length	i	:	送信データ長
	Author			TOA Corporation  K.Honda
	Revision		a00 2000/08/07
