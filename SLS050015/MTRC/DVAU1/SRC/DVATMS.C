...
static	char			befPadReset[8];	// 前回の非常通報器リセット要求の状態
...
int tms_receive_end( char *recvdata, short recvcnt, unsigned char status )
{
		...
		case 0x20:		/* SDR Format */
			...
			// 非常通報器のリセット要求  ( OFF->ON 要求 )
			for( i=0; i<8; i++ ){
				onoff = befPadReset[i] ^ recvp->SDR.PadReset[i];
				on = onoff & recvp->SDR.PadReset[i];
				DUALPORT.tms_request.PadReset[i] |= on;
				befPadReset[i] = recvp->SDR.PadReset[i];
			}
